...
- **시계**: 지역을 `/settings` 에서 최대 4곳까지 직접 등록 (위경도 기준).
...
헤더의 시계는 `/settings → 시계 지역` 에서 최대 **4곳**까지 등록합니다.

![시계 지역 설정](docs/screenshot-regions.png)

넣는 값은 **이름 · 배지 · 위경도 · 표준시 · 단위 · 날짜 표기** 뿐입니다.

- 날씨는 도시명이 아니라 **위경도**로 조회합니다 — 철자나 동명 도시에 걸리지 않습니다.
- 좌표 표기(`37.6°N · 127.0°E`)와 표준시 라벨(`EDT -4`, `UTC +9`, `UTC +5:45`)은 저장하지 않고
  그릴 때 만듭니다. 위경도만 고치면 표기도 따라옵니다. 서머타임도 그때그때 반영됩니다.
- 배지를 비우면 표준시에서 추측합니다(`Asia/Seoul` → `ASIA`).
- 표준시는 IANA 이름(`Asia/Seoul`, `America/New_York` …)이며 자주 쓰는 것은 자동완성에
  뜹니다. 목록에 없으면 직접 적어도 됩니다.
- 아무것도 설정하지 않으면 **서울 / West Lafayette** 두 곳이 기본입니다.
