    ...
    <section className="flex h-full min-h-0 flex-col rounded-[var(--radius-card)] bg-(--color-surface) p-5 ring-1 ring-(--color-border-soft)">
      ...
      {pins.length === 0 ? (
        <div className="rounded-lg border border-dashed border-(--color-border) py-6 text-center text-xs text-(--color-fg-4)">
          <Pin className="mx-auto mb-1.5 h-3.5 w-3.5" />
          핀이 없습니다
        </div>
      ) : (
        <ul className="flex flex-col gap-1.5">
          {pins.map((pin) => (
            <li
              key={pin.id}
              className="group flex items-center gap-2.5 rounded-lg bg-(--color-bg-2) p-2 ring-1 ring-(--color-border-soft) transition hover:bg-(--color-surface-hi)"
            >
              ...
              >
                ...
                <ExternalLink className="h-3 w-3 shrink-0 text-(--color-fg-4) opacity-0 group-hover:opacity-100" />
              </a>
              <button
                type="button"
