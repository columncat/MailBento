 ...
 *
 * 2 — 첨부 목록(attachments)·막은 픽셀 수(blockedTrackers·proxiedImages)가
 *     붙었고, 본문의 그림 주소가 우리 라우트로 바뀌었다. 1 로 담긴 행은
 *     첨부 목록이 없고 그림이 아직 발신자 서버를 직접 가리킨다 — 그대로
 *     내보내면 캐시가 적중한 메일만 조용히 발신자를 부른다.
 */
const FORMAT = 2;
